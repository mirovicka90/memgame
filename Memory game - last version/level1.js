...
function init() {
    ...
    // definisem niz u koji cu da smestam skorove igraca

    scores = [0, 0];

    // definisem niz u koji cu da smestam broj pokusaja igaraca(moves)

    m = [0, 0];

    // activePlayer je po logici na pocetku setovan na nulu

    activePlayer = 0;

    // takodje koristim flag gamePlaying i on je na pocetku false

    gamePlaying = false;

    // pomocu funkcije getQueryString upisujem vrednost varijabli player1Name

    player1Name = getQueryStringValue('player1');

    // pomocu funkcije getQueryString upisujem vrednost varijabli player2Name

    player2Name = getQueryStringValue('player2');

    // pomocu funkcije getQueryString upisujem vrednost varijabli count

    count = getQueryStringValue('timer');


    // na pocetku su skorevi igraca nula

    document.getElementById("score-0").textContent = '0';

    document.getElementById("score-1").textContent = '0';

    // imena igraca su jednaka player1Name i player2Name

    document.getElementById("name-0").textContent = player1Name;

    document.getElementById("name-1").textContent = player2Name;

    // counter dohvatamo po id iz HTML i kazemo da se on prikazuje kao counter + "secs"

    document.getElementById('counter').textContent = count + 'secs';


    // pozivam funkciju shuffle za mesanje niza images 

    shuffle(images);
}

// pozivam funkciju init za inicijalizaciju igrice

init();


// na pocetku ako je gamePlaying jednako false onda je gamePlaying jednako true, startuj tajmer
...
var clickOnDiv = function () {
    ...
    // dohvatam img element koji se nalazi u divu pomocu this.children i kazem prikazi se za jednu milisekundu

    $(this.children[0]).show(1000);

    // dohvatam img element koji se nalazi u divu pomocu this.children i kazem sakri se za cetiri milisekunde

    $(this.children[0]).hide(4000);

    // definisem currentIndex
    ...
    if (previousIndex != null && previousIndex !== currentIndex && images[previousIndex] === images[currentIndex]) {
        ...
        // vrsim proveru ukoliko se desi pogodak
        console.log("BINGO");

        // dohvatm dinamicki slike iz HTML za previousIndex i za currentIndex
